...
#' prediction; including \code{"interval"} in \code{what} will add to the
#' \code{data.frame}two additional columns with the lower and upper bounds of a
#' conditional prediction interval for each test prediction; and including
#' \code{"cons.interval"} will add to the \code{data.frame} two additional columns
#' with the lower and upper bounds of a conservative prediction interval for each
#' test prediction. Conservative prediction intervals are generated by breaking ties
#' in the empirical error distribution conservatively. They may be desirable when the
